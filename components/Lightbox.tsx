        ...
        <AnimatePresence>
            {item && (
                ...
                >
                    ...
                    >
                        <img src={item.image_url} alt={item.name} className="w-full h-auto object-contain max-h-[80vh]" />
                        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/70 to-transparent">
                            <h3 className="text-xl font-bold text-white font-serif">{item.name}</h3>
                            <p className="text-sm text-gray-200">{item.category}</p>
                             {item.product_code && (
                                <p className="text-xs text-gray-300 mt-1 font-mono bg-black/30 px-2 py-1 rounded-full inline-block">Code: {item.product_code}</p>
                            )}
                        </div>
                        <button
                            onClick={onClose}
