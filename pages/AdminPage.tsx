...
import { 
    ...
    getReviewImages, addReviewImages, deleteReviewImage, deleteAllReviewImages,
    getAboutImages, addAboutImages, deleteAboutImage, deleteAllAboutImages, getStorageUsage,
    getReviewImageCount, getAboutImageCount, getNewsletterSubscriberCount, getNewsletterSubscribers,
    deleteNewsletterSubscriber, deleteAllNewsletterSubscribers,
    getCategories, addCategory, deleteCategory, updateClothingItem
} from '../services/supabase';
import { ClothingItem, ReviewImage, AboutImage, NewsletterSubscription, Category, SiteConfig } from '../types';
import { 
...
type AdminPanel = 'stats' | 'items' | 'reviews' | 'about' | 'newsletter' | 'settings';
...
const ClothingItemsPanel: React.FC<{ onUpdate: () => void }> = ({ onUpdate }) => {
    ...
    const [newItem, setNewItem] = useState({ name: '', category: '', isForSale: false, price: '', isFeatured: false });
    ...
    const fetchItemsAndCategories = useCallback(async () => {
        ...
        try {
            ...
            if (categoryData.length > 0 && !newItem.category) {
                setNewItem(prev => ({ ...prev, category: categoryData[0].name }));
            }
        } catch (error: any) {
            toast.error(error.message);
        } finally {
    ...
    const handleAddItem = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!newItem.name || !newItem.category || !file) {
        ...
        try {
            const price = newItem.isForSale ? parseFloat(newItem.price) || null : null;
            await addClothingItem(newItem.name, newItem.category, file, newItem.isForSale, price, newItem.isFeatured);
            toast.success('Item added successfully!', { id: toastId });
            setNewItem({ name: '', category: categories[0]?.name || '', isForSale: false, price: '', isFeatured: false });
            setFile(null);
    ...
    const updateItem = async (id: number, updates: Partial<ClothingItem>, itemName: string) => {
        // Store the original state in case we need to revert
        const originalItems = [...items];
    
        // Optimistically update the UI. The user sees the change immediately.
        setItems(prevItems =>
            prevItems.map(item =>
        ...
        try {
            ...
            // On success, we do nothing. The UI is already correct. A silent success is best
            // for quick toggles to avoid notification spam.
        } catch (error: any) {
            // If the background update fails, revert the UI to the original state
            // and notify the user with a descriptive error message.
            setItems(originalItems);
            toast.error(`Could not update "${itemName}": ${error.message}`);
        }
    };
            ...
            <Panel title="Add New Clothing Item">
                 <form onSubmit={handleAddItem} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <input type="text" placeholder="Item Name" value={newItem.name} onChange={(e) => setNewItem({...newItem, name: e.target.value})} className="w-full p-2 border rounded bg-white text-brand-text" required />
                        <select value={newItem.category} onChange={(e) => setNewItem({...newItem, category: e.target.value})} className="w-full p-2 border rounded bg-white text-brand-text" required>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                        <div className="flex items-center gap-4">
                            <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={newItem.isForSale} onChange={(e) => setNewItem({...newItem, isForSale: e.target.checked})} /> For Sale</label>
                            <label className="flex items-center gap-2 cursor-pointer"><input type="checkbox" checked={newItem.isFeatured} onChange={(e) => setNewItem({...newItem, isFeatured: e.target.checked})} /> Featured</label>
                        </div>
                        <input type="number" placeholder="Price (NGN)" value={newItem.price} onChange={(e) => setNewItem({...newItem, price: e.target.value})} disabled={!newItem.isForSale} className="w-full p-2 border rounded disabled:bg-gray-100 bg-white text-brand-text" />
                    </div>
            ...
            <Panel title={`Manage Clothing Items (${items.length})`}>
                ...
                <div className="space-y-4 max-h-[80vh] overflow-y-auto pr-2">
                    {loading ? <p>Loading items...</p> : items.length === 0 ? <p>No items found.</p> :
                        items.map(item => (
                            <div key={item.id} className="flex items-start gap-4 p-2 border-b">
                                <img src={item.image_url} alt={item.name} className="w-20 h-24 object-cover rounded flex-shrink-0"/>
                                    ...
                                    <div className="flex items-center gap-4 mt-2">
                                        <label className="flex items-center gap-1.5 text-sm cursor-pointer">
                                            <input 
                                                ...
                                                onChange={e => updateItem(item.id, { is_for_sale: e.target.checked, price: e.target.checked ? item.price : null }, item.name)}
                                            /> For Sale
                                        </label>
                                        <input 
                                            type="number" 
                                            defaultValue={item.price ?? ''}
                                            onBlur={e => updateItem(item.id, { price: parseFloat(e.target.value) || null }, item.name)}
                                            disabled={!item.is_for_sale} 
                                            className="p-1 border rounded text-sm w-24 bg-white text-brand-text disabled:bg-gray-100" 
                                            placeholder="Price"
                                        />
                                    </div>
                                </div>
                                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                                    <button onClick={() => updateItem(item.id, { is_featured: !item.is_featured }, item.name)} className={`px-2 py-1 text-xs rounded-full ${item.is_featured ? 'bg-yellow-400 text-yellow-900' : 'bg-gray-200'}`}>Featured</button>
...
// --- Generic Image Management Panel ---
type PanelType = 'reviews' | 'about';
...
const ImageManagementPanel: React.FC<ImageManagementPanelProps> = ({ panelType, title, onUpdate }) => {
    const [images, setImages] = useState<(ReviewImage | AboutImage)[]>([]);
    const [loading, setLoading] = useState(true);
    ...
    const api = useMemo(() => ({
        get: panelType === 'reviews' ? getReviewImages : getAboutImages,
        add: panelType === 'reviews' ? addReviewImages : addAboutImages,
        delete: panelType === 'reviews' ? deleteReviewImage : deleteAboutImage,
        deleteAll: panelType === 'reviews' ? deleteAllReviewImages : deleteAllAboutImages,
    }), [panelType]);

    const fetchImages = useCallback(async () => {
        setLoading(true);
        try {
            setImages(await api.get());
        } catch (error: any) {
            toast.error(error.message);
        } finally {
            setLoading(false);
    ...
    const handleDelete = async (image: ReviewImage | AboutImage) => {
        if (window.confirm('Are you sure you want to delete this image?')) {
            try {
                await api.delete(image as any); // Type assertion is okay due to common structure
    ...
    const handleDeleteAll = async () => {
        if (window.confirm(`ARE YOU SURE you want to delete ALL images from "${title}"? This is permanent!`)) {
            const toastId = toast.loading('Deleting all images...');
            try {
                await api.deleteAll();
                toast.success('All images deleted.', { id: toastId });
                fetchImages();
                onUpdate();
            } catch (error: any) {
                toast.error(error.message, { id: toastId });
    ...
    return (
        <Panel title={title}>
            ...
            {images.length > 0 && <button onClick={handleDeleteAll} className="float-right -mt-4 text-sm text-red-500 hover:text-red-700">Delete All</button>}
            <h4 className="font-bold mb-4">Current Images ({images.length})</h4>
            {loading ? <p>Loading...</p> : (
                    ...
                    {images.map(img => (
                        <div key={img.id} className="relative group">
                            ...
                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                <button onClick={() => handleDelete(img)} className="p-2 bg-red-500 text-white rounded-full"><TrashIcon className="w-5 h-5"/></button>
                            </div>
                        </div>
                    ))}
                </div>
...
// --- Usage Monitor ---
const UsageMonitor: React.FC<{ stats: { totalImages: number; items: number; reviews: number; about: number; storage: number; subscribers: number } }> = ({ stats }) => {
    const { totalImages, items, reviews, about, storage, subscribers } = stats;
...
const AdminDashboard: React.FC = () => {
    ...
    const [stats, setStats] = useState({ totalImages: 0, items: 0, reviews: 0, about: 0, storage: 0, subscribers: 0 });
    const [loadingStats, setLoadingStats] = useState(true);

    const fetchAllStats = useCallback(async () => {
        setLoadingStats(true);
        try {
            const [itemCount, reviewCount, aboutCount, storage, subscriberCount] = await Promise.all([
                getClothingItemCount(), getReviewImageCount(), getAboutImageCount(), getStorageUsage(), getNewsletterSubscriberCount()
            ]);
            setStats({ totalImages: itemCount + reviewCount + aboutCount, items: itemCount, reviews: reviewCount, about: aboutCount, storage, subscribers: subscriberCount });
        } catch (err: any) {
    ...
    const navItems = [
        ...
        { id: 'about', label: 'About Images', icon: ImageIcon },
    ...
    const PanelContent = () => {
        switch (activePanel) {
            ...
            case 'reviews': return <ImageManagementPanel panelType="reviews" title="Review Images" onUpdate={fetchAllStats} />;
            case 'about': return <ImageManagementPanel panelType="about" title="About Page Images" onUpdate={fetchAllStats} />;
    ...
    // If a specific panel is selected, show its content with a back button.
    ...
    // Otherwise, show the main dashboard with cards.
    return (
        <div>
             <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="mt-8">
             ...
             <div className="mt-8">
                <button onClick={() => supabase.auth.signOut()} className="w-full md:w-auto flex items-center justify-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors text-red-600 bg-red-100/50 hover:bg-red-100">
