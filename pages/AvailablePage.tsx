...
import { getAvailableItems, getSetupError } from '../services/supabase';
...
const AvailableItemCard: React.FC<{ item: ClothingItem; onClick: () => void }> = ({ item, onClick }) => {
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-2 right-2">
                {item.price != null && (
                    <p className="text-lg font-bold text-white bg-brand-primary/60 px-3 py-1 rounded-full backdrop-blur-sm shadow-lg">
...
const AvailablePage: React.FC = () => {
        ...
        const fetchItems = async () => {
            try {
                ...
                if (setupMsg) {
                    ...
                    setError('Failed to fetch available items. The store might be setting up. Please check back later.');
                }
                console.error('Error fetching items on AvailablePage:', err.message || err);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div>
            <div className="text-center mb-12">
                <h1 className="text-4xl md:text-5xl font-serif font-bold text-brand-primary">Available for Sale</h1>
                <p className="mt-4 text-lg text-brand-secondary max-w-2xl mx-auto">
                    Browse our selection of products ready for purchase. Click on an item to see a larger view.
                </p>
            </div>
            ...
            {items.length > 0 ? (
                 <>
                    ...
                    >
                        {currentItems.map((item) => (
                            <AvailableItemCard key={item.id} item={item} onClick={() => setSelectedItem(item)} />
                        ))}
                    </motion.div>
                     {totalPages > 1 && (
                ...
                <div className="text-center py-20 text-brand-secondary">
                    <p>There are currently no items available for sale. Check back soon!</p>
                </div>
            )}
