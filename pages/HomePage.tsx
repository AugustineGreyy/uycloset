...
import React, { useState, useEffect } from 'react'; import { motion, AnimatePresence } from 'framer-motion'; import { Link } from 'react-router-dom'; import { getFeaturedItems, getClothingItems, getSetupError } from '../services/supabase'; import { ClothingItem } from '../types'; import { useWishlist } from '../contexts/WishlistContext'; import { HeartIcon } from '../components/Icons'; import Reviews from '../components/Reviews'; import toast from 'react-hot-toast'; import Lightbox from '../components/Lightbox';

const FEATURED_ITEMS_KEY = 'uy-closet-featured-items'; const FEATURED_ITEMS_TIMESTAMP_KEY = 'uy-closet-featured-items-timestamp'; const TWENTY_FOUR_HOURS_IN_MS = 24 * 60 * 60 * 1000;

const FeaturedItemCard: React.FC<{ item: ClothingItem; onClick: () => void; }> = ({ item, onClick }) => { const { addToWishlist, removeFromWishlist, isInWishlist } = useWishlist(); const isWishlisted = isInWishlist(item.id);
...
const FeaturedItemCardSkeleton = () => ( <div className="overflow-hidden rounded-lg shadow-lg bg-white flex flex-col animate-pulse"> <div className="bg-gray-200 aspect-w-4 aspect-h-5"></div> </div> );

const HomePage: React.FC = () => { const [featuredItems, setFeaturedItems] = useState<ClothingItem[]>([]); const [allItems, setAllItems] = useState<ClothingItem[]>([]); const [loading, setLoading] = useState(true); const [selectedItem, setSelectedItem] = useState<ClothingItem | null>(null); const [currentSlide, setCurrentSlide] = useState(0);
