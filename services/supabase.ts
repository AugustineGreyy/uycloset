...
import { AboutImage, ClothingItem, NewsletterSubscription, ReviewImage, Category, SiteConfig } from '../types';
...
//
// -> Error: "column clothing_items.is_featured does not exist"
//    ALTER TABLE public.clothing_items ADD COLUMN is_featured BOOLEAN DEFAULT false NOT NULL;
//
...
// -- Run this for each table you create: clothing_items, review_images, about_images, categories, etc.
...
// CREATE POLICY "Public read access" ON public.review_images FOR SELECT USING (true);
// CREATE POLICY "Public read access" ON public.about_images FOR SELECT USING (true);
...
// CREATE POLICY "Admin full access" ON public.review_images FOR ALL TO authenticated WITH CHECK (true);
// CREATE POLICY "Admin full access" ON public.about_images FOR ALL TO authenticated WITH CHECK (true);
...
// Fetch featured clothing items from the database
...
// Fetch available clothing items from the database
export const getAvailableItems = async (): Promise<ClothingItem[]> => {
    const { data, error } = await supabase
        .from('clothing_items')
        .select('*')
        .eq('is_for_sale', true)
...
export const getClothingItemsByIds = async (ids: number[]): Promise<ClothingItem[]> => {
    ...
    const dataById = new Map((data as ClothingItem[]).map(item => [item.id, item]));
...
export const addClothingItem = async (
    ...
    // 3. Insert new record, handling potential product_code collisions.
    let attempts = 0;
    const maxAttempts = 5;
...
// Update a specific clothing item (e.g., its featured status)
export const updateClothingItem = async (
    id: number,
    updates: Partial<ClothingItem>
): Promise<void> => {
    // This function now only performs the update and does not attempt to
    // return the updated data. This avoids potential RLS issues where the
    // SELECT policy might fail after an UPDATE. The UI will handle updates
    // "optimistically".
    const { error } = await supabase
        .from('clothing_items')
        .update(updates) // `as any` removed for better type safety.
        .eq('id', id);
...
export const deleteAllClothingItems = async (): Promise<void> => {
    ...
    const imagePaths = ((items || []) as any[]).map((item) => item.image_path).filter(path => !!path);
...
// Get total count of review images
export const getReviewImageCount = async (): Promise<number> => {
    const { count, error } = await supabase
        .from('review_images')
        .select('*', { count: 'exact', head: true });

    if (error) {
        console.error('Error counting review images:', error.message);
        throw error;
    }
...
// Get total count of about images
export const getAboutImageCount = async (): Promise<number> => {
    const { count, error } = await supabase
        .from('about_images')
        .select('*', { count: 'exact', head: true });
    
    if (error) {
        console.error('Error counting about images:', error.message);
        throw error;
    }
...
// Fetch all about images from the database
...
export const getWishlist = async (id: string): Promise<number[] | null> => {
    ...
    const typedData = data as { item_ids: Json; expires_at: string };
    ...
    const itemIds = typedData.item_ids;
...
export const getSiteConfig = async (): Promise<Record<string, any>> => {
    ...
    const config = (data || []).reduce((acc: any, { key, value }: any) => {
        acc[key] = value;
        return acc;
...
export const updateSiteConfig = async (configs: { key: string; value: any }[]): Promise<void> => {
...
// Generic function to add a visual asset (review or about image)
const addVisualAsset = async (
    table: 'review_images' | 'about_images',
    folder: 'reviews' | 'about',
    ...
    const uploadPromises = uploads.map(async (upload) => {
        ...
        if (uploadError) {
             ...
             throw uploadError; // Throw to be caught by Promise.allSettled
        }
        ...
        return { image_url: publicUrl, image_path: filePath, alt_text: altText };
    });
    ...
    if (successfulUploads.length > 0) {
        const insertPayload = successfulUploads.map(s => ({...s, alt_text: s.alt_text || undefined }));
        ...
        if (table === 'review_images') {
            ...
            const { error } = await supabase.from('about_images').insert(insertPayload);
            insertError = error;
        }
        ...
        if (insertError) {
            ...
            console.error('DB insert failed for batch, cleaned up storage:', insertError.message);
            throw insertError;
        }
    }
...
export const addAboutImages = (uploads: { file: File, altText: string | null }[]) => addVisualAsset('about_images', 'about', uploads);


// Generic function to delete a visual asset
const deleteVisualAsset = async (
    table: 'review_images' | 'about_images',
    image: ReviewImage | AboutImage
): Promise<void> => {
    if (image.image_path) {
        const { error: storageError } = await supabase.storage
            .from(BUCKET_NAME)
...
export const deleteAboutImage = (image: AboutImage) => deleteVisualAsset('about_images', image);
...
const deleteAllVisualAssets = async (
    ...
    const query = table === 'review_images'
        ? supabase.from('review_images').select('image_path')
        : supabase.from('about_images').select('image_path');
    ...
    const pathsToRemove = ((images || []) as any[]).map((img) => img.image_path).filter(Boolean);
...
export const deleteAllAboutImages = () => deleteAllVisualAssets('about_images');
